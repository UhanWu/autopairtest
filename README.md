...
Compares a continuous outcome across every pair of independent groups. For each pair, it uses all observations in those two groups as two unpaired samples and automatically selects:

- **Welch two-sample t-test** if Shapiro-Wilk normality is not rejected in either complete group (both p > 0.05)
- **Wilcoxon rank-sum** otherwise

Shapiro-Wilk is computed once for each complete group, not separately for every pair. A p-value above 0.05 indicates that normality was not rejected; it does not establish normality.
...
Compares a categorical outcome across every pair of independent groups. For each pair, it builds a group-by-outcome contingency table and automatically selects:
